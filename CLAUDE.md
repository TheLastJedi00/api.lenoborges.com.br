...
- **The email HTML carries no CSS, and that is a measurement, not taste** (spec 014). `renderEmail` emits a `<p>` per paragraph, an `<hr>`, the CTA as a plain link and the unsubscribe footer — no `style` attribute, no layout `<table>`, no background, no image, no `<h1>` repeating the subject. It has been diagrammed twice and both times Gmail filed the message under **Promoções**; with the Resend *Open/Click Tracking* already off, the A/B was run one suspect at a time and the styled template still landed in Promoções while the clean one landed in Principal. **There were two causes, not one** — the provider's 1×1 pixel and link rewriting, *and* the campaign-shaped markup — which is why removing either alone kept failing. Two trap tests in `email-template.spec.ts` fail on `style=`, `<table>`, `<img>`, `background`, `border-radius`, `padding` or the subject reappearing in the body, because the regression is invisible without them: nothing breaks, the send still works, and the cost shows up weeks later as open rate. The other half of the rule still holds: **the template is not the last thing that happens to the email** — the provider can rewrite the document, so its panel is checked before the code.
