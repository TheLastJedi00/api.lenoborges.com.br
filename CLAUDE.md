...
- **Two queries now need composite indexes in production** (`mural_questions` by week+votes, `badge_videos` by badge+kind+order). The emulator does not require indexes, so the suite stays green and the failure only shows up live.
