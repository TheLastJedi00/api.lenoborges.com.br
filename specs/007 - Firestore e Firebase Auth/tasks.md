...
- [] Task 03: Instalar `firebase-admin` e remover `@supabase/supabase-js` e `jose`. Arquivo:
...
- [] Task 04: Reescrever a validação de ambiente. Arquivo: `src/config/env.validation.ts`. Objetivo:
  ...
  três camadas abaixo.
- [] Task 05 (TDD): Escrever a spec do `FirebaseService`. Arquivo:
...
- [] Task 06: Implementar o `FirebaseService`. Arquivo: `src/auth/firebase.service.ts`. Objetivo:
...
- [] Task 01: Trocar as entidades por tipos com converter. Arquivos:
...
- [] Task 02 (TDD): Reescrever a spec do `WaitlistRepository`. Arquivo:
...
- [] Task 03: Implementar o `WaitlistRepository` sobre Firestore. Arquivo:
  ...
  uso.
- [] Task 04: Trocar o tratamento de corrida no service. Arquivo: `src/waitlist/waitlist.service.ts`.
...
- [] Task 05 (TDD + implementação): Trocar o `ProfileRepository`. Arquivos:
...
- [] Task 06: Escrever as security rules. Arquivo: `firestore.rules`. Objetivo: negar tudo. Só a API
...
- [] Task 07: Arrancar o TypeORM. Arquivos: `package.json`, `src/app.module.ts`, remoção de
...
- [] Task 01 (usuário): Configurar o console de **cada** projeto Firebase. Objetivo: em
  ...
  escolhido.
- [] Task 02 (TDD): Reescrever a spec do `signup`. Arquivo: `src/auth/auth.service.spec.ts`.
...
- [] Task 03: Implementar o `signup`. Arquivo: `src/auth/auth.service.ts`. Objetivo: some o
...
- [] Task 04: Apagar o caminho de definição de senha. Arquivos: `src/auth/auth.controller.ts`,
...
- [] Task 05: Atualizar a documentação da rota de cadastro. Arquivo: `src/auth/auth.controller.ts`.
...
- [] Task 01 (TDD): Reescrever a spec do `login`. Arquivo: `src/auth/auth.service.spec.ts`.
...
- [] Task 02: Implementar o `login`. Arquivo: `src/auth/auth.service.ts`. Objetivo: `expiresIn` do
  Firebase vem como string em segundos, não número — converter, senão o front recebe um tipo que o
  `Session` não declara.
- [] Task 03 (TDD): Reescrever a spec do `refresh`. Arquivo: `src/auth/auth.service.spec.ts`.
...
- [] Task 04: Implementar o `refresh`. Arquivo: `src/auth/auth.service.ts`. Objetivo: o `securetoken`
  devolve `user_id`, não o e-mail. O e-mail do `SessionResponseDto` passa a vir de
  `admin.auth().getUser(uid)`.
- [] Task 05 (TDD + implementação): Trocar o `logout`. Arquivos: `src/auth/auth.service.spec.ts`,
...
- [] Task 06 (TDD): Escrever a spec do guard novo. Arquivo:
...
- [] Task 07: Implementar o guard e apagar o antigo. Arquivos:
...
- [] Task 08: Atualizar os consumidores do guard. Arquivos: `src/auth/auth.module.ts`,
...
- [] Task 01: Apagar a página `/definir-senha`. Arquivos: `src/app/app.routes.ts` e o diretório
...
- [] Task 02: Apagar o que servia a essa página. Arquivos: `src/app/models/auth.model.ts`,
...
- [] Task 03: Abrir o diálogo de login pelo `?entrar=1`. Arquivos:
...
- [] Task 04: Ajustar o texto de confirmação do cadastro. Arquivo:
...
- [] Task 01: Apagar o diretório `supabase/` inteiro. Objetivo: `config.toml`, `templates/`,
...
- [] Task 02: Apagar o skill do Supabase. Arquivos: `.agents/skills/supabase/`, `skills-lock.json`.
...
- [] Task 01: Reescrever as regras 2 e 3 do `clauderc.md`. Arquivo: `.claude/clauderc.md`. Objetivo:
...
- [] Task 02: Reescrever o `CLAUDE.md`. Arquivo: `CLAUDE.md`. Objetivo: "Project state" e "Commands"
...
- [] Task 03: Reescrever a seção de auth do `README.md`. Arquivo: `README.md`. Objetivo: sai
...
- [] Task 04: Documentar o emulador. Arquivos: `firebase.json`, `README.md`, `package.json`.
...
- [] Task 05: Marcar as specs anteriores. Arquivos: `specs/004 - Acesso Antecipado/context.md`,
  ...
  existir.
- [] Task 06: Corrigir a "Resultado da aplicação" da spec 006. Arquivo: `specs/006 - Configuracao de
...
- [] Task 01: Unir as branches da spec e abrir o PR contra a `dev`. Objetivo: fluxo de versionamento
