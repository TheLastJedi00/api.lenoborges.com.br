# Fase 01: As redes sociais no perfil [ ]
...
- [ ] Task 01: Os dois campos na entidade. Arquivo: `src/profile/entities/profile.entity.ts`. Objetivo:
...
- [ ] Task 02 (TDD + implementação): Validar URL de rede social. Arquivos:
...
- [ ] Task 03 (TDD + implementação): Os campos no DTO. Arquivo: `src/profile/dto/update-profile.dto.ts`.
  ...
  suma, não que fique `''`.
- [ ] Task 04 (TDD + implementação): Gravar e devolver. Arquivos: `src/profile/profile.service.ts`,
...
- [ ] Task 05: `completedAt` continua intocado na edição. Objetivo: teste-trava de que editar o perfil de
...
- [ ] Task 01 (TDD + implementação): `reauthenticate(email, password)`. Arquivos:
...
- [ ] Task 02: O token fresco é carga útil, não sessão. Objetivo: o comentário registra que o `idToken`
...
- [ ] Task 03 (TDD): Reuso, não segundo mecanismo. Objetivo: teste-trava de que `login()` e
...
# Fase 03: Trocar de e-mail [ ]
Branch: `feat/013-trocar-email`

- [ ] Task 01: DTO. Arquivo: `src/profile/dto/change-email.dto.ts`. Objetivo: `newEmail` com
  `@IsEmail` e normalização por `normalizeEmail`, `password` obrigatória.
- [ ] Task 02 (TDD + implementação): O endpoint. Arquivos: `src/profile/profile.controller.ts`,
...
- [ ] Task 03 (TDD + implementação): E-mail novo igual ao atual. Objetivo: `400` antes de qualquer ida ao
  Firebase. Disparar confirmação para o endereço em que a pessoa já está é gastar um e-mail para não
  mudar nada.
- [ ] Task 04 (TDD + implementação): `EMAIL_EXISTS` não vaza. Objetivo: a resposta é a mesma mensagem
...
- [ ] Task 05: Throttle. Objetivo: `@Throttle({ default: { limit: 3, ttl: 60000 } })`. Sem ele, a decisão
  4 vira teatro: quem pode tentar mil vezes por minuto enumera do mesmo jeito, só mais devagar.
- [ ] Task 06 (TDD): Spec do endpoint. Objetivo: senha errada dá `401` **sem disparar e-mail nenhum** —
  a ordem é reautenticar primeiro, sempre. Sucesso dá `202`. E-mail igual ao atual dá `400`.

# Fase 04: Trocar de senha [ ]
Branch: `feat/013-trocar-senha`

- [ ] Task 01: DTO. Arquivo: `src/profile/dto/change-password.dto.ts`. Objetivo: `currentPassword` e
...
- [ ] Task 03 (TDD + implementação): Encerrar a sessão. Objetivo: `auth.revokeRefreshTokens(uid)` **e**
...
- [ ] Task 04 (TDD): A janela de uma hora está documentada, não escondida. Objetivo: o comentário aponta
...
- [ ] Task 01 (TDD + implementação): Anonimizar as perguntas. Arquivos:
...
- [ ] Task 02 (TDD + implementação): Apagar os votos dados. Arquivo: `src/mural/mural.repository.ts`.
...
- [ ] Task 03 (TDD + implementação): Apagar o perfil e a subcoleção. Arquivo:
...
- [ ] Task 04 (TDD + implementação): Apagar a entrada da lista de espera. Arquivos:
...
- [ ] Task 05 (TDD + implementação): O endpoint. Arquivos: `src/profile/profile.controller.ts`,
...
- [ ] Task 06 (TDD): A ordem é teste, não convenção. Objetivo: teste-trava de que `deleteUser` é a
...
- [ ] Task 07 (TDD + implementação): Admin não se exclui. Objetivo: `403` quando `role === 'admin'`
...
- [ ] Task 08 (TDD): Spec da exclusão, ponta a ponta. Objetivo: com um perfil, uma pergunta própria, um
...
- [ ] Task 01: Swagger. Objetivo: as quatro operações com `@ApiOperation` e `@ApiResponse` completos. A
...
- [ ] Task 02: `README.md`. Objetivo: uma seção curta sobre exclusão de conta — o que some, o que vira
  anônimo, e que é imediato e sem desfazer. É o texto que responde a pergunta de suporte antes de ela
  chegar.
- [ ] Task 03: A restrição da decisão 7, onde ela vai ser lida. Objetivo: registrar no
...
- [ ] Task 04: `npm test` verde e `npm run build` limpo.
