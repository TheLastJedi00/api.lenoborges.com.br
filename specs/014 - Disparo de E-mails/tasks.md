# Fase 01: O mailer, o template e a configuração [ ]
...
- [ ] Task 01: Dependência e variáveis. Arquivos: `package.json`, `.env.example`, `src/config/env.validation.ts`,
...
- [ ] Task 02: Módulo e porta. Arquivos: `src/emails/emails.module.ts`, `src/emails/mailer.service.ts`.
...
- [ ] Task 03 (TDD + implementação): O modo que não envia. Arquivo: `mailer.service.spec.ts`. Objetivo:
...
- [ ] Task 04 (TDD + implementação): O template. Arquivos: `src/emails/email-template.ts`, `.spec.ts`.
  ...
  nunca como marcação.
- [ ] Task 05 (TDD + implementação): O rodapé obrigatório. Objetivo: **não existe caminho que gere e-mail
...
- [ ] Task 06: Registrar o módulo. Arquivo: `src/app.module.ts`. Objetivo: `EmailsModule` nos imports,
...
- [ ] Task 01: Os três campos no perfil. Arquivos: `src/profile/entities/profile.entity.ts`, `.spec.ts`.
...
- [ ] Task 02 (TDD + implementação): O token. Arquivos: `src/emails/unsubscribe-token.ts`, `.spec.ts`.
...
- [ ] Task 03 (TDD + implementação): Gravar o descadastro. Arquivos: `src/profile/profile.repository.ts`,
...
- [ ] Task 04 (TDD + implementação): O endpoint público. Arquivos:
...
- [ ] Task 05: `@SkipThrottle` não; **throttle sim**. Objetivo: o endpoint é público e escreve — ele fica
...
- [ ] Task 06 (TDD + implementação): O interruptor do membro. Arquivos: `src/profile/profile.controller.ts`,
...
- [ ] Task 07 (TDD + implementação): `GET /me` devolve o estado. Arquivos: `src/profile/dto/*.ts`,
...
- [ ] Task 01 (TDD + implementação): O serviço de audiência. Arquivos:
...
- [ ] Task 02 (TDD + implementação): Os três cortes. Objetivo: fora quem tem `disabled: true`, quem tem
...
- [ ] Task 03 (TDD + implementação): Os filtros. Objetivo: `tiers` e `gradeMin`/`gradeMax`, aplicados
  **em memória** depois da junção (decisão 13). Teste-trava: filtro ausente significa **todos**, e nunca
  ...
  produção.
- [ ] Task 04 (TDD + implementação): Excluir o autor. Objetivo: `excludeUid` opcional, usado pelo gatilho
  de vídeo (decisão 7). Teste-trava: quem publicou não está na audiência do próprio anúncio.
- [ ] Task 05: DTO dos filtros. Arquivo: `src/emails/dto/audience-filter.dto.ts`. Objetivo: `tiers` com
  ...
  `paymentStatus`.
- [ ] Task 06 (TDD + implementação): `POST /admin/emails/audiencia`. Arquivos:
...
- [ ] Task 01: Entidade e converter. Arquivos: `src/emails/entities/email-campaign.entity.ts`. Objetivo:
...
- [ ] Task 02 (TDD + implementação): Repositório. Arquivos: `src/emails/email-campaign.repository.ts`,
...
- [ ] Task 03 (TDD + implementação): `ALREADY_EXISTS` engolido na campanha de vídeo. Objetivo: criar
  ...
  a base inteira.
- [ ] Task 04 (TDD + implementação): O envio em lotes. Arquivos: `src/emails/email-campaign.service.ts`,
...
- [ ] Task 05 (TDD + implementação): Cabeçalhos de lista. Objetivo: todo e-mail sai com
...
- [ ] Task 06 (TDD + implementação): O trinco. Objetivo: criar campanha com outra `enviando` responde
...
- [ ] Task 07 (TDD + implementação): Os endpoints do admin. Arquivos: `admin-emails.controller.ts`,
...
- [ ] Task 08: DTO da listagem. Objetivo: `GET /admin/emails` devolve assunto, tipo, contagens, estado e
...
- [ ] Task 09: Swagger. Objetivo: `@ApiTags('emails')`, `@ApiBearerAuth()` nos de admin, e o `POST` de
...
# Fase 05: O gatilho do vídeo [ ]
Branch: `feat/014-gatilho-video`

- [ ] Task 01 (TDD + implementação): Publicar vídeo dispara a campanha. Arquivos:
...
- [ ] Task 02: O assunto e o corpo do e-mail de vídeo. Objetivo: assunto com o título do vídeo, corpo com
...
- [ ] Task 03: Log na falha. Objetivo: o `catch` do gatilho loga com o id da campanha e o do vídeo. Sem
  ele, "às vezes não avisa" vira investigação sem pista — a mesma razão da Task 03 da Fase 03 da spec 012.
- [ ] Task 04: Comentário no gatilho. Objetivo: registrar que o e-mail é **acessório**, que nenhuma falha
...
# Fase 06: Bounce e reclamação [ ]
Branch: `feat/014-webhook`

- [ ] Task 01 (TDD + implementação): Verificação de assinatura. Arquivos:
...
- [ ] Task 02: `rawBody` no boot. Arquivo: `src/main.ts`. Objetivo: habilitar o corpo cru **só na rota do
...
- [ ] Task 03 (TDD + implementação): O endpoint. Arquivos: `src/emails/emails.controller.ts`, `.spec.ts`.
...
- [ ] Task 04 (TDD + implementação): Do e-mail para o `uid`. Objetivo: o webhook chega com o endereço, e o
...
- [ ] Task 05: `emailOptOut` no `AdminUserDto`. Arquivos: `src/admin/dto/admin-user.dto.ts`,
...
- [ ] Task 01: `CLAUDE.md`. Objetivo: três linhas novas na lista de garantias que vivem em código — **o
...
- [ ] Task 02: `README.md`. Objetivo: os oito endpoints na tabela, a coleção `email_campaigns` e os três
...
- [ ] Task 03: Autenticação do domínio. Objetivo: registrar no README o que precisa existir no DNS antes
  ...
  depois.
- [ ] Task 04 (e2e): O caminho inteiro contra o emulador. Arquivo: `test/emails.e2e-spec.ts`. Objetivo:
